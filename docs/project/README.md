...
The flow is deliberately one-directional: something is **noticed** (punchlist), then **understood with
receipts**, then becomes a **slice** (roadmap). Status lives in the roadmap row, nowhere else — no
second status file, no separate backlog.
