...
`0.7.0` published to the pre-release channel on release run 32084088671, in `entra` mode.

```text
Azure CLI login succeeds by using OIDC.
Authenticating with: --azure-credential   (vsce 3.9.2)
Publishing 'ewc3labs.excel-power-query-editor v0.7.0'...
Published ewc3labs.excel-power-query-editor v0.7.0.
```

Every link that had been inferred rather than observed is now observed:

| | |
| --- | --- |
| GitHub → Entra federation | the environment subject is accepted |
| `azure/login@v3` without a subscription | `allow-no-subscriptions` is sufficient |
| `vsce`'s credential chain | it **does** pick up the Azure CLI session — `AzureCliCredential` is second in the chain, and that was read from the source until now |
| publisher membership | the profile id with Contributor grants publish rights |
| pre-release channel | the VSIX carried `Microsoft.VisualStudio.Code.PreRelease = true`, verified in the manifest before approving |
| the human gate | the run parked on approval and went nowhere until a person clicked |

**The stable channel was untouched.** `vsce show` still reports `0.5.1` as latest, so the 5,450
installs saw nothing — which is the entire reason for publishing an odd minor first.

Open VSX warned and skipped, `OVSX_PAT` being unset, without failing the release.

### What this leaves

`0.8.0` to the stable channel is now a version bump and a tag. The authentication path it will use
has already published something.

**`oidc` mode remains unproven** and will stay that way until the Marketplace exposes its trust
policy. Switching to it is one variable, and the first `--oidc` publish will be its own first proof.
